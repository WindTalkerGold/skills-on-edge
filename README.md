...
A **skill** is a JSON file that defines a workflow of **actions**. When you trigger a skill, the extension runs each action in sequence, passing results between steps via a template engine.

Think of it as a pipeline: _page content → action 1 → action 2 → ... → result displayed to user_.
