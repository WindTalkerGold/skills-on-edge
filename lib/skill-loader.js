...
// Loads predefined skills from extension files and user skills from chrome.storage.local
...
const PredefinedSkills = {
  async getAll() {
    ...
    let files = [];
    try {
      const indexUrl = chrome.runtime.getURL('predefined-skills/index.json');
      const indexResp = await fetch(indexUrl);
      files = await indexResp.json();
    } catch (e) {
      console.warn('Failed to load predefined-skills/index.json', e);
      return skills;
    }
    for (const file of files) {
      try {
        const url = chrome.runtime.getURL(`predefined-skills/${file}`);
        const resp = await fetch(url);
        const skill = await resp.json();
        skills.push(skill);
      } catch (e) {
        console.warn(`Failed to load predefined skill: ${file}`, e);
      }
    }
    return skills;
  }
...
const UserSkills = {
  async getAll() {
    ...
    // Always try the bundled example
    filesToTry.add('csharp-simple-check.json');
